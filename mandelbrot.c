...
/*   Updated: 2023/08/26 13:30:51 by otuyishi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	grid_controller(void *param)
{
	t_complex	*c;
	t_fractol_m	m;
...
void	action_m(mlx_t *mlx_ptr, t_fractol_m *mt)
{
	...
	mt -> y_base = -1.5;
	mt -> y_roof = 1.5;
	mlx_loop_hook(mlx_ptr, grid_controller, mt);
...
void	run_mandelbrot(char *str)
{
	...
	mt->mlx_ptr = mlx_init(WIDTH, HEIGHT, "MLX42", 1);
	if (!mt)
		return ;
	if (!m.mlx_ptr)
	{
